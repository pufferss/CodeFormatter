...
class CPP:
    ...
    def GetFunctions(self):
        L = re.findall(r"[a-zA-Z][a-zA-Z0-9_&*<>:]* [a-zA-Z][a-zA-Z0-9_]+\(.*\)\{", self.Statements)
        L = [re.sub(r"\(.*", "", x.split()[1]) for x in L]  # comment to keep prototype etc
        #L = [x for x in L if x != "main"]
        return list(set(L))

    def GetClasses(self):
        return [x.split()[1][0:-1] for x in re.findall(r"class [a-zA-Z][a-zA-Z0-9_]+\{", self.Statements)]

    def GetStructs(self):
        return [x.split()[1][0:-1] for x in re.findall(r"struct [a-zA-Z][a-zA-Z0-9_]+\{", self.Statements)]
